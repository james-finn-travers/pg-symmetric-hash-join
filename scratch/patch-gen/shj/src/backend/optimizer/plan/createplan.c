					 ...
					 Plan *inner_plan)
{
	...
	/* Strip excess columns from both relations before hashing */
	disuse_physical_tlist(inner_plan, best_path->jpath.innerjoinpath);
	disuse_physical_tlist(outer_plan, best_path->jpath.outerjoinpath);

	/*
	 * Symmetric hash join builds hash tables on both inputs, so wrap
	 * inner and outer plans in Hash nodes.
	 */
