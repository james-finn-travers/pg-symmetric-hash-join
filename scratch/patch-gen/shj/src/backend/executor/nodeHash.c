 ...
 *		ExecHash
 *
 *		Pipelined query execution support for Symmetric Hash Join.
 *		Returns tuples one at a time to support non-blocking streaming.
 * ----------------------------------------------------------------
 */
TupleTableSlot *
ExecHash(HashState *node)
{
	...
	/* Get the subplan node producing the input tuples */
	outerNode = outerPlanState(node);

	/* Get the hash table associated with this state node */
	hashtable = node->hashtable;

	if (hashtable == NULL)
		return NULL;

	/* Fetch the next tuple from the subplan */
	slot = ExecProcNode(outerNode);

	/* Return NULL if child subplan is exhausted */
	if (TupIsNull(slot))
		return NULL;

	econtext = node->ps.ps_ExprContext;

	/* Store tuple in expression context for hash key evaluation */
	econtext->ecxt_innertuple = slot;

	/* Compute the hash value using the appropriate hash keys */
	hashvalue = ExecHashGetHashValue(hashtable, econtext, node->hashkeys);

	/* Insert the physical tuple into the hash table memory structures */
						...
						int *numbatches)
{
	...
	/*
	 * Force single-batch execution for Symmetric Hash Join by hardcoding
	 * the batch count to 1. Pipelining requires both tables to fit in memory.
	 */
	nbatch = 1;
	...
	{
		/* We'll need multiple buckets but keep to 1 batch */
		long		lbuckets;
...
/*
 * ExecScanHashBucket
 *		Scan a hash bucket for matches to the current join key.
 *
 * Modified to support Symmetric Hash Join: reads hj_CurrentTable to determine 
 * whether to scan/probe the inner hash table (0) or the outer hash table (1).
 */
HeapTuple
				   ...
				   ExprContext *econtext)
{
	...
	HashJoinTuple hashTuple;
	uint32		hashvalue;
	int			tableId = hjstate->hj_CurrentTable;  /* 0 = scan inner, 1 = scan outer */

	/*
	 * Select the target hash table, its hash value, and the current tuple
	 * position based on the active scan direction:
	 *   0 = scan inner hash table (matching outer tuple keys)
	 *   1 = scan outer hash table (matching inner tuple keys)
	 */
	if (tableId == 0)
	{
	...
	else if (tableId == 1)
	{
		...
		hashvalue = hjstate->hj_OuterHashValue;
		hashTuple = hjstate->hj_OuterCurTuple;
	}
	else
	{
		elog(ERROR, "ExecScanHashBucket: invalid hj_CurrentTable selector %d",
			 tableId);
		return NULL;
	}

	/*
	 * Determine the start position within the hash bucket.
	 * If the current tuple cursor is NULL, start scanning from the bucket header,
	 * otherwise resume from the next element in the collision chain.
	 */
	if (hashTuple == NULL)
	{
		int bucketNo;
		if (tableId == 0)
			bucketNo = hjstate->hj_InnerBucketNo;
		else
			bucketNo = hjstate->hj_OuterBucketNo;
	...
	while (hashTuple != NULL)
	{
		if (hashTuple->hashvalue == hashvalue)
		{
			...
			TupleTableSlot *scannedSlot;
			TupleTableSlot *targetSlot;

			/* Use the appropriate tuple slot depending on the table being probed */
			if (tableId == 0)
				targetSlot = hjstate->hj_HashTupleSlot;
			else
				targetSlot = hjstate->hj_OuterHashTupleSlot;
			
			scannedSlot = ExecStoreTuple(heapTuple,
									  targetSlot,
									  InvalidBuffer,
									  false);  /* do not pfree */
			...
			/*
			 * Set expression context:
			 *   tableId == 0: Probing inner table -> scanned tuple is the inner tuple
			 *   tableId == 1: Probing outer table -> scanned tuple is the outer tuple
			 */
			if (tableId == 0)
				econtext->ecxt_innertuple = scannedSlot;
			else
				econtext->ecxt_outertuple = scannedSlot;

			/* Reset query evaluation temporary memory to prevent leaks */
			ResetExprContext(econtext);

			if (ExecQual(hjclauses, econtext, false))
			{
				/* Update the matching tuple cursor to allow resuming the scan */
				if (tableId == 0)
